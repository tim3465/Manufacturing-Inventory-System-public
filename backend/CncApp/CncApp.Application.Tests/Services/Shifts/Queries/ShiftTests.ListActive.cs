...
{
    [Fact]
    public async Task ListActiveAsync_WhenShiftsExist_ReturnsDtos()
    {
        ...
        var result = await ShiftService.ListActiveAsync(cancellationToken);
    ...
    [Fact]
    public async Task ListActiveAsync_WhenNoShiftsExist_ReturnsEmptyList()
    {
        var cancellationToken = CancellationToken.None;
        var shifts = new List<Shift>();
