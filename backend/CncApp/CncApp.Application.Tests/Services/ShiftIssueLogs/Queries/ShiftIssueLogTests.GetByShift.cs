...
{
    [Fact]
    public async Task GetByShiftAsync_WhenLogsExist_ReturnsMappedDtos()
    {
        // Arrange
        var shiftId = 1;
        ...
        var logs = new List<ShiftIssueLog>
        {
            ...
            new ShiftIssueLog(shiftId, IssueTypeEnum.Production, 0, "Downtime issue", TimeSpan.FromMinutes(30)) { Id = 2 }
        };

        var expectedDtos = new List<ShiftIssueLogResultDto>
        {
            new ShiftIssueLogResultDto { Id = 1, ShiftId = shiftId, IssueType = IssueTypeEnum.Setup, ScrapQuantity = 5, Description = "Scrap issue" },
            new ShiftIssueLogResultDto { Id = 2, ShiftId = shiftId, IssueType = IssueTypeEnum.Production, ScrapQuantity = 0, Description = "Downtime issue", Downtime = TimeSpan.FromMinutes(30) }
        };
