...
{
    public async Task<JobProductionSearchResultDto> SearchProductionAsync(
        JobProductionSearchRequestDto request, CancellationToken ct = default)
    {
        var (items, totalCount) = await _jobRepository.SearchProductionAsync(request, ct);
        ...
        return new JobProductionSearchResultDto
        {
            Items = dtos,
