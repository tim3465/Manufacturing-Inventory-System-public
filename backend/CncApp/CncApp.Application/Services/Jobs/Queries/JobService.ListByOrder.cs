...
{
    public async Task<List<JobProductionDto>> ListByOrderAsync(int orderId, CancellationToken ct = default)
    {
        var jobs = await _jobRepository.ListByOrderWithShiftsAsync(orderId, ct);
