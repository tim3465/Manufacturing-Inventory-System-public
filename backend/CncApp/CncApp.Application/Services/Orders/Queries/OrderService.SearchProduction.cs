...
{
    public async Task<OrderProductionSearchResultDto> SearchProductionAsync(
        OrderProductionSearchRequestDto request, CancellationToken ct = default)
    {
        var (items, totalCount) = await _orderRepository.SearchActiveWithDetailsAsync(request, ct);
        ...
        return new OrderProductionSearchResultDto
        {
            Items = dtos,
